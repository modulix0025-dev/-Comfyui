...
def _write_ready_sidecar(file_path: str, slot_id=None, run_id: str = "") -> None:
    ...
    CHANGE 5.1 — Added `run_id` parameter (Bug 4 fix, cross-run contamination).
    The per-run identifier is written into the sidecar payload so the disk-
    accumulation branch of SmartSaveImageMegaNode can reject files from
...
def _read_ready_sidecar(file_path: str):
    """
    CHANGE 5.2 — Self-contained sidecar reader (Bug 4 fix dependency).
...
class SmartSaveImageMegaNode:
    ...
    def INPUT_TYPES(cls):
        ...
        # CHANGE 5.3 — run_id (Bug 4 fix). forceInput ensures the value
        # can only reach this node via backend injection from lgutils
        # (CHANGE 1.5) and is never persisted as a user-editable widget
        # value in the serialized workflow. Empty default keeps this
        # node fully compatible with workflows that don't use the
        # GroupExecutor pipeline.
    ...
    def save_mega(self, prompt=None, extra_pnginfo=None, trigger_in=None,
        ...
        # ──────────────────────────────────────────────────────────────
        # CHANGE 5.4 — run_id handling (Bug 4 fix).
        ...
        #     files live in the legacy IMAGE_SUBFOLDER directly.
        # The `effective_subfolder` value is what we report in
        # ui.images / slot_dashboard so the frontend /view requests
        # resolve correctly.
        # ──────────────────────────────────────────────────────────────
        effective_subfolder = (
        ...
        for i in range(1, NUM_SLOTS + 1):
            ...
            if received_flag:
                ...
                if batch is not None and len(batch) > 0:
                    ...
                    if self._save_png_atomic(batch[0], full_path, metadata):
                        ...
                        #
                        # CHANGE 5.4 — include run_id in the sidecar so
                        # subsequent group runs can gate their disk
                        # accumulation on it (Bug 4 fix).
            ...
            # ──────────────────────────────────────────────────────────
            # CHANGE 5.4 — run_id gating for disk accumulation (Bug 4).
            # If we didn't save a fresh frame for this slot BUT a file
