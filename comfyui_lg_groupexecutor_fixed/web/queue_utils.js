...
class QueueManager {
  ...
  initializeHooks() {
    ...
    api.queuePrompt = async function(index, prompt) {
      if (this.queueNodeIds && this.queueNodeIds.length && prompt.output) {
        ...
        let newOutput = {};
        // Pass 1 — upstream walk from each queued output node (existing).
        for (const queueNodeId of this.queueNodeIds) {
          this.recursiveAddNodes(String(queueNodeId), oldOutput, newOutput);
        }
        // Pass 2 — DOWNSTREAM BFS with dangling-ref stripping.
        // BUG #1 FIX (normal path): when SmartSaveImageMega / SmartImagePackagerFinal
        // sit OUTSIDE the queued group and consume outputs from inside, they must
        // also be captured or the save/packaging chain never runs.
        this._collectDownstreamWithStrip(oldOutput, newOutput);
  ...
  // BUG #1 FIX (normal path): Repeatedly scan `oldOutput` for nodes whose
  // inputs reference a node already in `newOutput`. Add them with cloned
  // inputs where any dangling reference (to a node NOT in newOutput) is
  // stripped. This preserves optional inputs cleanly so SmartImagePackagerFinal
  // can validate with only the path_XX slots that correspond to the current
  // filtered subset.
  _collectDownstreamWithStrip(oldOutput, newOutput) {
    let changed = true;
    while (changed) {
      changed = false;
      for (const [nid, node] of Object.entries(oldOutput)) {
        if (newOutput[nid] != null) continue;
        const inputs = node.inputs || {};
        let touches = false;
        for (const v of Object.values(inputs)) {
          if (Array.isArray(v) && v.length >= 1 && newOutput[String(v[0])] != null) {
            touches = true;
            break;
          }
        }
        ...
        for (const [k, v] of Object.entries(inputs)) {
          if (Array.isArray(v) && v.length >= 1) {
            if (newOutput[String(v[0])] != null) clonedInputs[k] = v;
            // else: drop dangling reference
          } else {
            clonedInputs[k] = v;
          }
        }
        newOutput[nid] = { ...node, inputs: clonedInputs };
        changed = true;
      }
    }
    return newOutput;
