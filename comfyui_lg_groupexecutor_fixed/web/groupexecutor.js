...
class GroupExecutorNode extends BaseNode {
    ...
    async executeGroups() {
        ...
        try {
            ...
            if (!this.properties.isCancelling) {
                // BUG #4 FIX — run the final packaging step so the Packager
                // sees paths from ALL groups and emits one complete ZIP.
                try {
                    this.updateStatus(`Finalizing package...`);
                    const ran = await this.executeFinalPackaging();
                    if (ran) {
                        console.log('[GroupExecutor] 最终打包步骤已完成');
                    }
                } catch (finalErr) {
                    console.error('[GroupExecutor] 最终打包步骤出错:', finalErr);
                }
    ...
    // BUG #4 FIX — Final packaging step (frontend path).
    // After all groups have run, find every SmartImagePackagerFinal /
    // SmartVideoPackagerFinal node in the full graph prompt, collect them plus
    // their entire UPSTREAM tree (which includes every SmartSaveImageMega and
    // its upstream chain from every group), and submit that as one final
    // prompt via the normal ComfyUI queue mechanism. The packager then sees
    // every saved path from every group and produces the complete ZIP.
    async executeFinalPackaging() {
        try {
            const { output: fullApiPrompt } = await app.graphToPrompt();
            const packagerIds = Object.entries(fullApiPrompt)
                .filter(([, n]) =>
                    n && (n.class_type === "SmartImagePackagerFinal" ||
                          n.class_type === "SmartVideoPackagerFinal"))
                .map(([id]) => String(id));

            if (packagerIds.length === 0) return false;

            // Use the normal ComfyUI queue mechanism — queueManager.queueOutputNodes
            // sets queueNodeIds, the api.queuePrompt hook filters to
            // packagers + upstream via recursiveAddNodes, and the prompt is
            // submitted through ComfyUI's own scheduler (no seed randomization,
            // so cached KSampler outputs are reused and images are preserved).
            await queueManager.queueOutputNodes(packagerIds);
            await this.waitForQueue();
            return true;
        } catch (err) {
            console.error(`[GroupExecutor] executeFinalPackaging failed:`, err);
            return false;
        }
    }
    computeSize() {
