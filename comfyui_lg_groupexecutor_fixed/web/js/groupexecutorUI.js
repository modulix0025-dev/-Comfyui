...
class GroupExecutorUI {
    ...
    async executeGroupViaBackend(groupName, nodeIds) {
        try {
            ...
            // Pass 2 — BUG #1 FIX: downstream BFS with dangling-ref stripping
            // so Save/Packager nodes placed outside the group are captured.
            let changed = true;
            while (changed) {
                changed = false;
                for (const [nid, node] of Object.entries(fullApiPrompt)) {
                    ...
                    for (const [k, v] of Object.entries(inputs)) {
                        if (Array.isArray(v) && v.length >= 1) {
                            if (filteredPrompt[String(v[0])]) cloned[k] = v;
                        } else cloned[k] = v;
                    }
                    filteredPrompt[nid] = { ...node, inputs: cloned };
                    changed = true;
                }
            }
