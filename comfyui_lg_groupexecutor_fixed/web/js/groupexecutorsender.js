...
app.registerExtension({
    ...
    async beforeRegisterNodeDef(nodeType, nodeData, app) {
        if (nodeData.name === "GroupExecutorSender") {
            ...
            // ─────────────────────────────────────────────────────────
            // CHANGE 2.8 — group_executor_group_complete listener
            // (MASTER PROMPT 2, RC4 fix — per-group progress UX).
            //
            // Backend emits this event after each group's
            // _wait_for_completion returns (lgutils.py CHANGE 2.2).
            // Payload: { node_id, group_name, run_id, was_interrupted,
            //            completed_count, total_count }
            //
            // For GroupExecutorSender nodes, render the same per-group
            // progress text the GroupExecutorNode already renders via
            // its own _onGroupComplete handler. Sender nodes don't have
            // _onGroupComplete (they're not virtual), so we update the
            // node title directly via updateStatus() — the sender's
            // existing public method.
            //
            // Filter: only act when this node is currently executing
            // and the event matches by node_id. Multiple sender nodes
            // on the same canvas must not cross-talk.
            // ─────────────────────────────────────────────────────────
            api.addEventListener("group_executor_group_complete", ({ detail }) => {
                if (!detail || !detail.node_id) return;
                const node = app.graph._nodes_by_id[detail.node_id];
                if (!node) return;
                if (!node.properties || !node.properties.isExecuting) return;
                try {
                    ...
                    if (typeof node.updateStatus === "function") {
                        node.updateStatus(`${glyph} ${done}/${total} - ${name}`);
                    }
                } catch (err) {
                    console.warn('[GroupExecutorSender] group_executor_group_complete handler failed:', err);
                }
            });
        }
    }
