"""
sync_utils — atomic I/O, sidecar handshake, slot inference.
...
Responsibilities:
  ...
  2. Sidecar handshake (.ready.json)
       ...
       Payload:  filename, mtime, size, slot_id, status, written_at, run_id.
...
MTIME_TOLERANCE   = 5.0   # seconds — safer margin for fast SSDs running back-to-back group executions
STABILITY_TRIES   = 5
...
def write_ready_sidecar(file_path, slot_id=None, run_id=""):
    """
    ...
    `run_id` is stamped in so the disk-accumulation branch in
    SmartSaveImageMegaNode/SmartSaveVideoMegaNode can distinguish files
    saved during THIS run from files left over by earlier runs. Absent/empty
    run_id is permitted for backward compatibility.
    """
