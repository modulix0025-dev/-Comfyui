...
def write_ready_sidecar(file_path, slot_id=None, run_id=""):
    ...
    CHANGE 3.1 — `run_id` (Bug 4 fix, cross-run contamination).
    A short per-run identifier is stamped into every sidecar so the disk
    accumulation branch in SmartSaveImageMegaNode can distinguish files
    saved during THIS run from files left over by earlier runs. Absent/
    empty run_id is permitted for backward compatibility — existing
    validators ignore unknown fields, and the accumulation logic only
    gates when the CURRENT run has a non-empty run_id.
    """
