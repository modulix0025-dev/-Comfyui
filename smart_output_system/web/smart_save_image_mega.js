...
app.registerExtension({
    ...
    async beforeRegisterNodeDef(nodeType, nodeData) {
        ...
        nodeType.prototype.onExecuted = function (message) {
            ...
            // CHANGE 4.2 — Accept BOTH "slots" and "slot_dashboard" UI
            // keys. Historically the batch variant emitted "slots" while
            // the fan-in variant's standalone JS expects "slot_dashboard".
            // Now that the Python side emits BOTH keys (CHANGE 2.6/2.7),
            // this handler works whether it receives payloads from the
            // old batch variant or the newer unified variant.
...
// ─────────────────────────────────────────────────────────────────────────────
// CHANGE 4.1 — Register the fan-in variant SmartSaveImageMegaNode (Bug 3B).
//
// The original extension above only handled "SmartSaveImageMega" (batch).
// The fan-in variant "SmartSaveImageMegaNode" was ONLY served by the
// SmartSaveImageMega/ package's standalone JS — and only if that package
// won node-type registration. This block adds a second registration so the
// smart_output_system JS also renders a dashboard for the fan-in variant,
// meaning the preview works regardless of which Python class ends up
// winning registration when both packages are installed together.
// ─────────────────────────────────────────────────────────────────────────────
app.registerExtension({
    ...
    async beforeRegisterNodeDef(nodeType, nodeData) {
        ...
        nodeType.prototype.onExecuted = function (message) {
            origExecuted?.apply(this, arguments);
            // Accept BOTH "slot_dashboard" and "slots" for compatibility
            // with every upstream Python implementation.
