// smart_output_system/web/smart_video_packager_final.js
// Adds a "Download ZIP" button below the SmartVideoPackagerFinal node.
...
function buildWidget(node) {
    ...
    btn.addEventListener("click", (e) => {
        ...
        const name = basenameFromPath(s.zip_path) || "videos.zip";
...
function updateWidget(node, state) {
    ...
    if (state && state.ready && state.download_url) {
        ...
        ui.btn.textContent = `Download ZIP (${n} clip${n === 1 ? "" : "s"})`;
...
app.registerExtension({
    name: "smart_output_system.packager_video_download",
    async beforeRegisterNodeDef(nodeType, nodeData) {
        if (nodeData.name !== "SmartVideoPackagerFinal") return;
